...
export interface UseModuleVersionsOptions {
  moduleId: number;
  autoFetch?: boolean;
  initialFilters?: VersionFilters;
}

export interface UseModuleVersionsReturn {
  ...
  createVersion: (data: CreateVersionRequest) => Promise<ModuleVersion>;
  updateVersion: (version: string, data: UpdateVersionRequest) => Promise<ModuleVersion>;
  publishVersion: (version: string) => Promise<ModuleVersion>;
  getVersion: (version: string) => Promise<ModuleVersion>;
  refreshVersion: (version: string) => Promise<void>;
  getLatestVersion: () => ModuleVersion | null;
  getPublishedVersions: () => ModuleVersion[];
  getDraftVersions: () => ModuleVersion[];
}
...
export function useModuleVersions(options: UseModuleVersionsOptions): UseModuleVersionsReturn {
  const { moduleId, autoFetch = true, initialFilters = {} } = options;

  const [state, setState] = useState<ModuleVersionState>({
    versions: [],
    loading: false,
    error: null,
  });
  ...
  const fetchVersions = useCallback(async () => {
    ...
    try {
      const response = await moduleService.listVersions(moduleId, filters);
      setState((prev) => ({
        ...prev,
  ...
  const createVersion = useCallback(
    async (data: CreateVersionRequest): Promise<ModuleVersion> => {
      try {
        setState((prev) => ({ ...prev, loading: true, error: null }));

        const newVersion = await moduleService.createVersion(moduleId, data);
  ...
  const updateVersion = useCallback(
    async (version: string, data: UpdateVersionRequest): Promise<ModuleVersion> => {
      try {
        setState((prev) => ({ ...prev, loading: true, error: null }));

        const updatedVersion = await moduleService.updateVersion(moduleId, version, data);
  ...
  const publishVersion = useCallback(
    async (version: string): Promise<ModuleVersion> => {
      try {
        setState((prev) => ({ ...prev, loading: true, error: null }));

        const publishedVersion = await moduleService.publishVersion(moduleId, version);
  ...
  const getVersion = useCallback(
    async (version: string): Promise<ModuleVersion> => {
      try {
        const versionData = await moduleService.getVersion(moduleId, version);
        return versionData;
      } catch (error: any) {
        const apiError = error as ApiError;
        setState((prev) => ({
  ...
  const refreshVersion = useCallback(
    async (version: string) => {
      try {
        const updatedVersion = await moduleService.getVersion(moduleId, version);
        setState((prev) => ({
          ...prev,
          versions: prev.versions.map((v) => (v.version === version ? updatedVersion : v)),
        }));
  ...
  const getLatestVersion = useCallback((): ModuleVersion | null => {
  ...
  const getPublishedVersions = useCallback((): ModuleVersion[] => {
    return state.versions.filter((version) => version.published);
  }, [state.versions]);

  // Get draft versions only
  const getDraftVersions = useCallback((): ModuleVersion[] => {
    return state.versions.filter((version) => !version.published);
  }, [state.versions]);

  // Auto-fetch on mount and when filters change
  useEffect(() => {
    if (autoFetch && moduleId) {
      fetchVersions();
    }
  }, [autoFetch, moduleId, fetchVersions]);
